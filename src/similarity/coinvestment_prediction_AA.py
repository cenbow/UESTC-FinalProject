...
def calculate_sim_for_specified_pairs(train_graph, test_graph, node):
    ...
    for pair in its.combinations(node, r=2):
        score.append(sim_AA(train_graph, pair[0], pair[1]))
...
def roc_plot(score, true_value):
    ...
    plt.savefig("sim_AA_ROC&AUC(test).png")
...
score,true_value = calculate_sim_for_specified_pairs(train_graph, test_graph, investors)
